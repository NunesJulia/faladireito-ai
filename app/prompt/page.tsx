...
export default function PromptPage() {
  ...
  const handleSendPrompt = async () => {
    ...
    const token = session?.user?.email; // você pode usar session.accessToken quando configurar JWT
    ...
    try {
      const res = await fetch(process.env.NEXT_PUBLIC_PROMPT_CONSUME!, {
        ...
        headers: {
          ...
          Authorization: `Bearer ${token}` // enviando o token se for necessário
        },
        body: JSON.stringify({
      ...
      setMessage(`Prompt enviado com sucesso! Resposta: ${JSON.stringify(data)}`);
      setPromptText("");

    } catch (error) {
      console.error(error);
      setMessage("Erro ao enviar prompt. Tente novamente mais tarde.");
    } finally {
      setIsSending(false);
    }
    setTimeout(() => {
      setIsSending(false);
      setMessage(`Prompt enviado com sucesso! (Usuário: ${session.user?.name})`);
      setPromptText("");
    }, 1500);
  };
